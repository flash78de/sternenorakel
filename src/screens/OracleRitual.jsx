...
export default function OracleRitual() {
  ...
  const cards = {
    icon: (
      <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="#E8C77A" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round">
  ...
  if (premium)
    return (
      <button className="ritual" onClick={() => nav('/oracle/draw', { state: { ritual } })}>
        ...
        <span style={{ flex: 1, minWidth: 0 }}>
          <span style={{ display: 'block', fontFamily: 'var(--font-head)', color: 'var(--gold-1)', fontSize: 18, fontWeight: 600 }}>{title}</span>
