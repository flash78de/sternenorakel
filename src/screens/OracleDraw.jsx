...
export default function OracleDraw() {
  ...
  const [saved, setSaved] = useState(null) // Ergebnis von saveEntry (enthält id)
      ...
      setTimeout(() => {
        ...
        timers.current.push(
          setTimeout(() => {
            ...
            const res = saveEntry(msg, '') // Auto-Speichern: nichts geht verloren
            setMessage(msg)
            setSaved(res)
            setPhase('message')
          }, 1700)
  ...
  // Zurück zum Dashboard – Belohnung nur bei frischer Ziehung zeigen
  const finish = () => {
    const reward = saved && !saved.already && (saved.constellation || saved.rankUp) ? { ...saved } : null
    ...
    return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '14px 28px 34px' }}>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <button
            className="dice-orb anim-orb"
            onClick={draw}
            aria-label="Sternenwürfel werfen"
          >
  ...
  if (phase === 'listening')
    return (
      <div style={{ position: 'relative', flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', textAlign: 'center' }}>
        ...
        {/* Fortschritts-Funkeln über allem */}
  ...
  // ---------- error: sanfter Zwischenzustand (kein „App-Fehler") ----------
  if (phase === 'error')
    return (
  ...
  // ---------- message: stärkere Hierarchie, bereits gespeichert ----------
  ...
  // ---------- reflection: Notiz optional, aktualisiert denselben Eintrag ----------
